...
>
	...
	>
		<div class="flex h-full w-full flex-col items-center justify-center gap-2 text-slate-700">
			...
			<span class="text-[10px] font-bold tracking-[0.2em] text-slate-600 uppercase">No preview</span
			>
		</div>
		...
		<div class="absolute top-3 left-3 flex items-center gap-2">
			<span
				class={cn(
					'flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase backdrop-blur-md',
					isPublic
			...
			<span
				class={cn(
					'rounded-full border px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase backdrop-blur-md',
					status.class
				)}
			>
				{status.label}
		...
		<div class="absolute top-3 right-3">
			...
			{#if menuOpen}
				...
				></button>
				<div
					class="absolute right-0 z-50 mt-2 w-40 overflow-hidden rounded-xl border border-white/10 bg-surface p-1 shadow-2xl"
				>
					{#if !draft}
						<button
							onclick={(e) => {
