	...
	>
		<div transition:fly={{ y: 20, duration: 300 }} class="w-full max-w-lg overflow-hidden rounded-[2rem] border border-white/5 bg-surface shadow-2xl">
			...
			<div class="relative aspect-video w-full overflow-hidden bg-gradient-to-br from-slate-800 to-slate-950">
				...
				<span
					class={cn(
						'absolute left-4 top-4 flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur-md',
				...
				<button onclick={onClose} aria-label="Close" class="absolute right-4 top-4 rounded-full bg-black/50 p-2 text-slate-300 backdrop-blur-md transition-colors hover:text-white">
					<X size={18} />
				</button>
			</div>
			...
			<div class="flex flex-col gap-4 p-6 sm:p-8">
				...
				<div class="flex items-center gap-2 text-sm font-medium text-slate-400">
					<Layers size={16} />
					{quiz.cardCount} {quiz.cardCount === 1 ? 'card' : 'cards'}
				</div>
