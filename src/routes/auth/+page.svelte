<script lang="ts">
	...
	let passwordMismatch = $derived(
		activeTab === 'create' && confirmPassword.length > 0 && password !== confirmPassword
	);
	...
	function validateForm(): boolean {
		...
		if (password.length < 6) {
			fieldErrors.password = 'Min 6 characters required';
			isValid = false;
		}
		if (activeTab === 'create') {
			if (username.length < 3) {
				fieldErrors.username = 'Username too short';
				isValid = false;
			}
...
<div
	class="flex min-h-dvh w-full flex-col items-center bg-background p-4 pt-[10dvh] transition-all sm:pt-[15dvh]"
>
	<!-- min-h-dvh -->
	...
	<div
		class="h-fit w-full max-w-110 overflow-hidden rounded-4xl border border-white/5 bg-surface p-6 shadow-2xl sm:p-10"
	>
		<!-- max-w-110, rounded-4xl -->

		<!-- Tabs -->
		<div class="mb-8 flex rounded-xl border border-white/5 bg-slate-950/80 p-1.5">
			<button
		...
		>
					...
					<a
						...
						class={cn(
							activeTab !== 'signin'
								...
								: 'text-[10px] font-semibold tracking-wider text-primary uppercase transition-colors hover:text-primary-hover'
						)}
						aria-hidden={activeTab !== 'signin'}>Forgot password?</a
					>
				</div>
			</div>
			...
			<Button type="submit" class="mt-2 w-full py-4 text-base" isLoading={loading}>
		...
		<div class="my-10 flex items-center gap-4">
			...
			<span
				class="text-[10px] font-medium tracking-[0.2em] whitespace-nowrap text-slate-500 uppercase"
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
			<Button
				variant="secondary"
				class="rounded-lg border-white/5 px-4 py-3 font-bold"
				onclick={() =>
					(window.location.href = `${env.PUBLIC_API_URL}/auth/oauth2/authorization/google`)}
			>
				<svg class="mr-2 h-5 w-5 shrink-0" viewBox="0 0 48 48"
					><path
						fill="#EA4335"
			...
			<Button
				...
				class="rounded-lg border-white/5 px-4 py-3 font-bold"
				onclick={() =>
					(window.location.href = `${env.PUBLIC_API_URL}/auth/oauth2/authorization/github`)}
			>
				<svg class="mr-2 h-5 w-5 shrink-0 fill-current" viewBox="0 0 16 16"
					><path
