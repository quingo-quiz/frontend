...
<div class="flex min-h-dvh w-full flex-col items-center pt-[10dvh] sm:pt-[15dvh] p-4 bg-background transition-all"> <!-- min-h-dvh -->
	...
	<div class="w-full max-w-110 rounded-4xl border border-white/5 bg-surface p-6 sm:p-10 shadow-2xl overflow-hidden h-fit"> <!-- max-w-110, rounded-4xl -->
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<Button variant="secondary" class="font-bold py-3 px-4 rounded-lg border-white/5" onclick={() => window.location.href = `${env.PUBLIC_API_URL}/auth/oauth2/authorization/google`}>
			...
			<Button variant="secondary" class="font-bold py-3 px-4 rounded-lg border-white/5" onclick={() => window.location.href = `${env.PUBLIC_API_URL}/auth/oauth2/authorization/github`}>
