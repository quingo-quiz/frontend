<script lang="ts">
	...
	let showConfirmRevokeSingle = $state<null | {
		tokenId: string;
		title?: string;
		message?: string;
	}>(null);
...
<div class="animate-in fade-in slide-in-from-bottom-4 space-y-8 pb-20 duration-500">
	...
	{:else if !securityContext.status}
		<div
			class="rounded-4xl border border-dashed border-white/10 bg-surface py-20 text-center text-slate-500"
		>
			<p class="mb-2 text-lg font-bold">Error Loading Security Data</p>
	...
	{:else}
		...
		<div class="rounded-4xl border border-white/5 bg-surface p-6 shadow-2xl sm:p-10">
			<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div class="flex min-w-0 items-center gap-3">
					<div
						class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/5 bg-slate-950 text-primary"
					>
						<Lock size={20} />
					</div>
					<div class="min-w-0">
				...
				<Button
					...
					class={cn(
						'h-10 px-5 text-[10px] font-bold tracking-widest uppercase sm:shrink-0',
			...
			>
				...
				<span
					class={cn(
						'rounded-full border px-3 py-1 text-[9px] font-bold tracking-widest whitespace-nowrap uppercase',
						securityContext.status.passwordSet
		...
		<div class="rounded-4xl border border-white/5 bg-surface p-6 shadow-2xl sm:p-10">
			<div class="mb-6 flex items-center gap-3">
				<div
					class="flex h-10 w-10 items-center justify-center rounded-xl border border-white/5 bg-slate-950 font-bold text-primary shadow-inner"
				>
					<ShieldCheck size={20} />
				</div>
			...
			<div
				class="flex flex-col gap-5 rounded-2xl border border-white/5 bg-slate-950/50 p-6 transition-all hover:border-white/10 sm:flex-row sm:items-center sm:justify-between"
			>
				<div class="flex items-center gap-4">
					<div
						class="flex h-12 w-12 items-center justify-center rounded-xl border border-white/5 bg-slate-900 shadow-inner"
					>
						<Monitor
							class={securityContext.status.mfaEnabled ? 'text-primary' : 'text-slate-600'}
						/>
					</div>
					<div>
				...
				<div class="flex flex-col items-start gap-3 sm:items-end">
					<button
						...
						class={cn(
							...
							securityContext.status.mfaEnabled
								? 'bg-primary shadow-[0_0_15px_rgba(249,115,22,0.3)]'
								: 'bg-slate-800'
						)}
					>
						<span
		...
		<div class="rounded-4xl border border-white/5 bg-surface p-6 shadow-2xl sm:p-10">
			<div class="mb-8 flex items-center justify-between">
				<div class="flex items-center gap-3">
					<div
						class="flex h-10 w-10 items-center justify-center rounded-xl border border-white/5 bg-slate-950 text-primary"
					>
						<Monitor size={20} />
					</div>
				...
				<button
					...
					class="flex items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-2 text-[10px] font-bold tracking-widest text-red-500 uppercase shadow-lg shadow-red-500/5 transition-all hover:bg-red-500 hover:text-white disabled:cursor-not-allowed disabled:opacity-30"
				>
					{#if isRevokingAll}
						<div
						...
						>
							...
							<div class="flex flex-1 items-center gap-4">
								...
								<div
									class={cn(
										...
										isCurrent
											...
											: 'border border-slate-800 bg-slate-900 text-slate-500 group-hover:bg-slate-800 group-hover:text-primary'
									)}
								>
								...
								<div class="min-w-0 space-y-2">
									...
									<div class="flex flex-wrap items-center gap-2">
										<p class="text-xs font-bold tracking-tight text-slate-100 uppercase">
											{session.browser} on {session.os}
										</p>
										{#if isCurrent}
											<span
												class="rounded-full border border-green-500/30 bg-green-500/10 px-2 py-0.5 text-[9px] font-bold tracking-wider whitespace-nowrap text-green-500 uppercase"
											>
												Your Device
											</span>
										{/if}
							...
							{#if !isCurrent}
								<button
									...
									class="ml-4 rounded-lg p-2.5 text-slate-500 transition-all hover:bg-red-500/10 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-30"
								>
