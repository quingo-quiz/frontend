...
{:else if userContext.user}
	...
	<div class="mx-auto max-w-3xl">
		<div class="rounded-[2.5rem] border border-white/5 bg-surface p-6 shadow-2xl sm:p-10">
			...
			>
				<div
					class="flex h-24 w-24 shrink-0 items-center justify-center rounded-[2rem] bg-primary text-4xl font-bold text-white shadow-2xl shadow-primary/30 sm:h-28 sm:w-28 sm:text-5xl"
				>
				...
				<div class="min-w-0 grow space-y-2">
					<div class="flex flex-wrap items-center justify-center gap-2.5 sm:justify-start">
						...
						{#each user.roles as role}
							<span
								class="rounded-full border border-primary/20 bg-primary/10 px-2.5 py-0.5 text-[10px] font-bold tracking-widest text-primary uppercase"
							>
								{role}
							</span>
						{/each}
					...
					<div
						class="flex items-center justify-center gap-2 text-sm text-slate-400 sm:justify-start"
					>
							...
							<Tooltip width="w-60">
								...
								{#snippet panel()}
									...
									<p class="mt-1 text-slate-400">
										Confirm your email to unlock full account access.
									</p>
									<button
										type="button"
										onclick={handleResendVerification}
										disabled={isResendingVerification}
										class="mt-2.5 inline-flex items-center gap-1.5 rounded-lg border border-amber-400/30 bg-amber-500/10 px-3 py-1.5 text-[11px] font-bold tracking-wider text-amber-200 uppercase transition-colors hover:bg-amber-500/20 disabled:opacity-50"
									>
				...
				<a
					href="/settings"
					class="hidden shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-2.5 text-xs font-bold tracking-wider text-slate-200 uppercase transition-all hover:bg-white/10 hover:text-white sm:inline-flex"
				>
			...
			<div class="mt-8 border-t border-white/5 pt-6">
				<p class="mb-2 text-[10px] font-bold tracking-widest text-slate-500 uppercase">About</p>
				{#if user.bio}
					<p class="max-w-2xl text-sm leading-relaxed text-slate-300">{user.bio}</p>
				{:else}
					<p class="flex items-center gap-2 text-sm text-slate-600 italic">
						<Pencil size={14} />
						No bio yet. Add one in
						<a href="/settings" class="font-semibold text-primary not-italic hover:underline"
							>Settings</a
						>.
					</p>
				{/if}
			...
			<a
				href="/settings"
				class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-xs font-bold tracking-wider text-slate-200 uppercase transition-all hover:bg-white/10 hover:text-white sm:hidden"
			>
				<SettingsIcon size={15} /> Settings
			</a>
		</div>
