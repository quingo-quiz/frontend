<script lang="ts">
	import Button from '$lib/components/ui/Button.svelte';
	import Input from '$lib/components/ui/Input.svelte';
	import { Mail, Lock, ArrowRight } from 'lucide-svelte';
	import { cn } from '$lib/utils/ui';

	let email = $state('');
	let password = $state('');
	let loading = $state(false);
	let activeTab = $state('signin');

	async function handleLogin() {
		loading = true;
		await new Promise(r => setTimeout(r, 1500));
		loading = false;
	}
</script>

<div class="flex min-h-screen flex-col items-center justify-center p-4 sm:p-6 bg-background">
	<div class="flex flex-col items-center mb-10 text-center">
		<h1 class="text-4xl font-bold tracking-tight text-white italic">Quingo</h1>
	</div>

	<!-- Auth Card -->
	<div class="w-full max-w-[440px] rounded-[2rem] border border-white/5 bg-surface p-6 sm:p-10 shadow-2xl">
		
		<!-- Tabs -->
		<div class="flex p-1.5 bg-background rounded-xl mb-8">
			<button 
				onclick={() => activeTab = 'signin'}
				class={cn("flex-1 py-2 text-sm font-semibold rounded-lg transition-all", 
				activeTab === 'signin' ? "bg-primary text-white shadow-md" : "text-slate-500 hover:text-slate-300")}
			>
				Sign In
			</button>
			<button 
				onclick={() => activeTab = 'create'}
				class={cn("flex-1 py-2 text-sm font-semibold rounded-lg transition-all", 
				activeTab === 'create' ? "bg-primary text-white shadow-md" : "text-slate-500 hover:text-slate-300")}
			>
				Create Account
			</button>
		</div>

		<form class="space-y-5" onsubmit={(e) => { e.preventDefault(); handleLogin(); }}>
			<Input 
				bind:value={email} 
				label="Email Address" 
				placeholder="you@example.com" 
				icon={Mail} 
			/>
			
			<div class="space-y-2">
				<Input 
					bind:value={password} 
					type="password" 
					label="Password" 
					placeholder="Enter your password" 
					icon={Lock} 
				/>
				<div class="flex justify-end px-1">
					<button type="button" class="text-[10px] font-semibold text-primary hover:text-primary-hover transition-colors uppercase tracking-wider">
						Forgot password?
					</button>
				</div>
			</div>

			<Button type="submit" class="w-full py-4 text-base mt-4 shadow-lg shadow-primary/20" isLoading={loading}>
				Sign In <ArrowRight size={18} class="ml-1" />
			</Button>
		</form>

		<!-- ТОТ САМЫЙ РАЗДЕЛИТЕЛЬ -->
		<div class="flex items-center gap-4 my-10">
			<div class="flex-grow border-t border-white/5"></div>
			<span class="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-500 whitespace-nowrap">
				Or continue with
			</span>
			<div class="flex-grow border-t border-white/5"></div>
		</div>

		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<Button variant="secondary" class="font-bold py-3 px-4 rounded-lg border-white/5" onclick={() => {}}>
				<svg class="w-5 h-5 mr-2 shrink-0" viewBox="0 0 48 48">
					<path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
					<path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
					<path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24s.92 7.54 2.56 10.78l7.97-6.19z"/>
					<path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.31-8.16 2.31-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
				</svg>
				Google
			</Button>
			<Button variant="secondary" class="font-bold py-3 px-4 rounded-lg border-white/5" onclick={() => {}}>
				<svg class="w-5 h-5 mr-2 shrink-0 fill-current" viewBox="0 0 16 16">
					<path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/>
				</svg>
				GitHub
			</Button>
		</div>
	</div>
</div>